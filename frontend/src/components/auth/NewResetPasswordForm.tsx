...
export function NewResetPasswordForm({ onSuccess }: NewResetPasswordFormProps) {
  ...
  useEffect(() => {
    ...
    const verifyToken = async () => {
      ...
      try {
        ...
        toast('Verifying reset link...', { icon: '🔍' })
        
        const result = await passwordResetService.verifyResetSession()
        ...
        if (result.success) {
          ...
          console.error('Reset session verification failed:', result.error)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log('Form validation passed, starting password update...')
    setIsLoading(true)
    setErrors({})

    try {
      console.log('Calling passwordResetService.updatePassword...')
      const result = await passwordResetService.updatePassword(formData.password)

      console.log('Password update result:', result)

      if (result.success) {
        console.log('Password update SUCCESS - setting success state')
        setIsSuccess(true)
